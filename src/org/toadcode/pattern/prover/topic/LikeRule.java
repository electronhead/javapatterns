...
public class LikeRule extends TopicRule implements InstanceRule {
     ...
     */
    public LikeRule(String string) {
        this.string = string;
    }

    public boolean implies(Rule rule) {

        if (rule instanceof LikeRule) {
            String s1 = string;
            String s2 = ((LikeRule) rule).string;

            return stringImpliesString(s1, s2);
        }

        if (rule instanceof NotLikeRule) {
            String s1 = string;
            String s2 = ((NotLikeRule) rule).string;
            boolean unequal = !(stringImpliesString(s1, s2)
                    || stringImpliesString(s1, s2));

            return unequal && !stringIntersectsString(s1, s2);
        }
